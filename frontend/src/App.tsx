...
const App: React.FC = () => {
  ...
  const [greenRoomAction, setGreenRoomAction] = useState<
    "watch" | "panel" | "browse" | undefined
  >(undefined);
  ...
  const requestMedia = async (targetTab?: Tab) => {
    try {
      ...
      addNotification(
        "Access Granted",
        "Camera and Microphone are ready for production.",
      );
    } catch (err) {
  ...
  const { isConnected } = useWebSocket(
    useCallback(
      (msg: WebSocketMessage) => {
        addNotification(
          msg.title || "New Alert",
          msg.body || "You received a notification.",
        );
      },
      [addNotification],
    ),
  ...
  const renderContent = () => {
    switch (activeTab) {
      case "home":
        return <Dashboard initialAction={greenRoomAction} />;
      case "browse":
        return (
          <Browse
            onWatch={(id) => {
  ...
  const navItems: { id: Tab; icon: React.ElementType; label: string }[] = [
    ...
    { id: "audio", icon: Mic, label: "Rooms" },
    { id: "marketplace", icon: Clapperboard, label: "Market" },
    ...
    <div className="flex flex-col md:flex-row h-screen bg-deep-dark overflow-hidden text-slate-200">
      ...
      <AnimatePresence>
        {!hasInteracted && (
          <GreenRoom
            onEnter={(config) => {
              setGreenRoomAction(config.action);
              setHasInteracted(true);
              if (config.action === "watch" || config.action === "panel") {
                setActiveTab("home");
              }
              addNotification(
                "Live Staging Complete",
                "Entry parameters synchronized.",
              );
            }}
          />
        )}
        ...
        <div className="flex flex-col items-center gap-2 mt-4 w-full px-3 pt-4 border-t border-white/5">
          {isAdmin && (
            <button
              ...
              aria-label="Overlord Terminal"
              title="Overlord Terminal"
              className={`w-full p-3.5 rounded-2xl transition-all flex flex-col items-center gap-1.5 ${
                activeTab === "admin"
                  ? "nav-active text-color-gold"
                  : "text-slate-500 hover:text-color-gold hover:bg-white/5"
              }`}
            >
              <Shield size={20} />
